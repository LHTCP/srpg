...
1. 행동 순서 패널 분리: 상단 HUD와 별도 initiative/turn order tracker의 정보 경계를 P2에서 확정
2. 총기 발포 방향/조준 문법 재정의: 기본 공격/오버워치/발포 연출이 모두 8방향 고정처럼 보이는 문제를 P2에서 검토
3. 맵 메이커 방향성 엄폐 segment 편집 UI
4. 초기 4인 고유 패시브/대표 스킬 최종 이름, 전직 연계, 밸런스 수치 확정
5. 특수 지형 상호작용의 복합 효과와 승리 조건 연동
6. 공용 전투 태그/패링/총기 파급 브릿지 수치 밸런스 검증
7. 메이커 화면 UX: 효과유형 드롭다운 지연 재현과 필드 의미 툴팁은 P3로 유지
