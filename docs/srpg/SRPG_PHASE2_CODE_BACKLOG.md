...
- PlayMode 테스트: HUD의 반응 상태 표기(`반응: 준비/소모/예약`), 상단 헤더/좌측 콘솔/하단 유닛 카드/행동 preview 카드, 태세/방향/오버클럭/재장전/엄폐/상호작용 직접 조작, 범례, 오버워치 버튼, hover 문구, 로그 핵심 문구, tile overlay marker/ring/border 존재와 유닛 ring 아래 높이 계약을 스모크 검증
...
- `M1OpeningPrototype` 화면 관찰 표본에서 확인된 북쪽 사격 루트/남쪽 신호 장치 루트 혼동은 데이터 보정 없이 `TBD-012` overlay 문법으로 대응했다. 이동/공격/ZOC/상호작용을 색상만 다른 채움이 아니라 중심 marker/테두리/ring/objective marker로 분리한다.
