...
## 2026-06-15 전투 HUD 사용성 보정

### 명령 콘솔/설명/스킬 drawer 분리

- 좌측 전술 콘솔은 명령 전용 레일로 유지한다.
- 유닛 설명, hover preview, 행동 예상 정보는 좌측 콘솔 옆의 좁은 보조 칸이 아니라 하단 `ActiveUnitCardPanel`과 `ActionPreviewPanel`로 라우팅한다.
- 스킬 선택은 좌측 콘솔 내부에 끼워 넣지 않고 별도 `SkillSelectionDrawerPanel`로 띄운다.
- 스킬 drawer는 `닫기` 버튼과 `스킬 사용` 버튼 재클릭 닫힘을 모두 지원한다.
- 로그 패널은 기본 접힘 상태로 시작하고, 접힘 상태에서는 작은 재열기 레일만 화면 공간을 차지한다.
