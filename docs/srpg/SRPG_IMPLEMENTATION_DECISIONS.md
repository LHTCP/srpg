...
- 공격/위험 범위는 외곽선/다이아몬드형 밀집 표시 대신 낮은 채도의 중심 marker를 사용한다.
- 이동 가능 marker보다 위험 marker를 약간 크고 낮은 채도로 두되, ZOC warning ring과 경계태세 테두리와는 mesh 문법을 분리한다.
- PlayMode와 관찰 테스트는 공격/위험 overlay가 `SrpTileOverlayCenterDisc` mesh를 사용하는지 검증한다.

### 후속 의사결정

- 경계태세 사격 VFX/애니메이션, marker 투명도/펄스, 정식 초상/행동 순서 아트는 실제 에디터 플레이와 아트 에셋이 생긴 뒤 별도 튜닝한다.
