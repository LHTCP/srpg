...
- [x] 행동 순서 패널 분리 (`TBD-011`)
  - 현재 상단 HUD에 섞인 턴/행동 순서 정보를 별도 상단 우측 initiative/turn order icon strip으로 분리했다.
  - 현재 유닛은 더 큰 아이콘과 포인터로 강조하고, 다음 3~5명은 런타임 생성 얼굴 토큰으로 미리보기한다.
  - 상단 HUD는 라운드, 상태, 위험영역, 맵 요약 중심으로 줄였고, 새 스트립은 로그 패널 자식이 아닌 별도 캔버스 UI로 배치한다.
  - PlayMode에서 패널 존재, 로그 패널과의 분리, 현재 유닛 아이콘 강조, 다음 순서 preview, 턴 진행 후 갱신을 검증한다.
- [x] 타일 overlay 시각 문법 개편 (`TBD-012`)
  - 이동 가능 범위는 타일 전체 채움 대신 타일 중심 원 marker로 분리했다.
  - 공격/위험 범위는 외곽 danger 테두리, ZOC는 얇은 warning ring으로 분리했다.
  - 오버워치/엄폐는 별도 테두리, 패링은 warning ring, 스킬/상호작용은 marker 계열로 분리해 색상만 다른 채움 문법을 제거했다.
  - `신호 장치`는 노랑 objective marker로 표시하고, 모든 tile overlay marker는 PR #61 유닛 발밑 ring보다 낮은 높이에 둔다.
- [ ] 전투 UX 추가 피드백 후속 (`TBD-014`, `BUG-001`, `TBD-012` 후속)
  - 사용자 노출 명칭은 `오버워치`에서 `경계태세`로 변경한다. 코드 식별자 `SrpOverwatch`는 별도 리팩터링 전까지 유지 가능하지만 HUD, 로그, 버튼, floating text, 문서의 플레이어-facing 문구는 `경계태세`를 기준으로 교체한다.
  - `경계태세` 발동 가이드 텍스트 후보를 선정한다. 예: 예약 시 `경계태세 준비`, 발동 시 `경계사격!` 또는 `경계태세 발동!`, 해제 시 `경계태세 해제`. 최종 문구는 실제 화면에서 길이와 가독성을 확인해 고른다.
  - `BUG-001`: 경계태세/ReactionShot으로 사망한 유닛이 즉시 제거되거나 HP 0/사망 상태로 렌더링되지 않는 지연 이슈를 재현하고 수정한다. 발동 로그, world-space feedback, 유닛 mesh/HUD/행동 순서 갱신 순서를 PlayMode로 고정한다.
  - 공격/위험 범위의 다이아몬드형 외곽선은 실제 화면에서 과하게 어지럽고 시각 소음이 크다는 피드백이 있다. 공격 가능 범위는 전체 다이아몬드 선 대신 더 조용한 marker, 낮은 밀도 edge, hover 중심 표시, 또는 실제 위협 타일만 강조하는 방식으로 재검토한다.
