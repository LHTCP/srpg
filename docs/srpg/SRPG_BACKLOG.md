...
- [x] 타일 overlay 시각 문법 개편 (`TBD-012`)
  ...
  - 공격/위험 범위는 1차에서 외곽 danger 테두리로 분리했고, 후속 피드백 기준으로 더 조용한 중심 marker 문법으로 낮췄다.
  - 경계태세/엄폐는 별도 테두리, 패링은 warning ring, 스킬/상호작용은 marker 계열로 분리해 색상만 다른 채움 문법을 제거했다.
  - `신호 장치`는 노랑 objective marker로 표시하고, 모든 tile overlay marker는 PR #61 유닛 발밑 ring보다 낮은 높이에 둔다.
- [x] 전투 UX 추가 피드백 후속 (`TBD-014`, `BUG-001`, `TBD-012` 후속)
  - 사용자 노출 명칭은 `경계태세`로 교체했다. 코드 식별자 `SrpOverwatch`는 유지하지만 HUD, 버튼, 로그, floating text, QA 프리셋 표시명은 `경계태세`/`경계사격` 기준이다.
  ...
  - 공격/위험 범위는 외곽선/다이아몬드형 과밀 표현 대신 낮은 채도 중심 marker를 사용한다. ZOC warning ring, 경계태세 테두리, 이동 중심점과 다른 mesh 계약을 PlayMode/관찰 테스트로 고정했다.
- [ ] 경계태세/공격·위험 overlay 후속 아트 튜닝
  - 정식 VFX/파티클, marker 크기/투명도/애니메이션, 경계태세 사격 연출은 실제 에디터 플레이 피드백과 아트 에셋 기준으로 별도 결정한다.
