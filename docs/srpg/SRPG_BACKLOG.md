...
- [x] 타일 overlay 시각 문법 개편 (`TBD-012`)
  ...
  - 공격/위험 범위와 경계태세 범위는 실제 피드백 기준으로 반투명 타일막 문법을 사용한다.
  - 엄폐는 별도 테두리, 패링은 warning ring, 스킬/상호작용은 marker 계열로 분리해 의미가 섞이지 않게 했다.
...
- [x] 전투 UX 추가 피드백 후속 (`TBD-014`, `BUG-001`, `TBD-012` 후속)
  ...
  - 파란 선 다이아몬드가 경계태세 범위였음을 확인하고, 공격/위험 범위와 경계태세 범위를 모두 반투명 타일막으로 조정했다. ZOC warning ring, 이동 중심점, 상호작용 marker와 다른 계약을 PlayMode/관찰 테스트로 고정했다.
- [ ] 경계태세/공격·위험 overlay 후속 아트 튜닝
  - 정식 VFX/파티클, 타일막 투명도/애니메이션, 경계태세 사격 연출은 실제 에디터 플레이 피드백과 아트 에셋 기준으로 별도 결정한다.
