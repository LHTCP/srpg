...
32. 전술 HUD 사용성 보정: 명령 전용 좌측 콘솔, 독립 스킬 선택 drawer, 기본 접힘 로그 레일, 하단 카드 중심 설명/preview 계약 추가

다음 구현 순서:

1. `M1OpeningPrototype` 실제 플레이/AI 시뮬레이션으로 첫 전투의 적 수, 배치 간격, 루트별 위험도를 검증
2. 메이커/맵 에디터 UX에서 엄폐 segment와 상호작용 포인트 편집 방식 검토
3. 초기 4인 고유 패시브/대표 스킬 최종 수치와 전직 연계 확정
