...
38. 전투 UX 추가 피드백 후속: 오버워치 사용자 노출 명칭을 `경계태세`로 교체하고, 경계태세 발동 문구를 화면 기준으로 선정하며, 경계태세 사망 렌더링 지연과 공격/위험 범위 다이아몬드 시각 소음을 PlayMode/관찰 QA로 검증

다음 구현 순서:

1. 전투 UX 추가 피드백을 처리한다. `경계태세` 명칭 교체, 발동 문구 선정, 경계태세 사망 렌더링 지연 수정, 공격/위험 범위 표시 문법 재검토를 한 묶음으로 본다.
2. 메이커/맵 에디터 UX에서 엄폐 segment와 상호작용 포인트 편집 방식을 검토한다.
3. 초기 4인 고유 패시브/대표 스킬 최종 수치와 전직 연계를 확정한다.
4. 공용 전투 태그/패링/총기 파급 브릿지 수치 밸런스를 검증한다.
5. 메이커 효과유형 드롭다운 성능과 필드 의미 툴팁은 P3 UX로 재현/설계한다.
...
- 타일 overlay 세부 튜닝: `TBD-012` 1차 문법은 중심 marker/테두리/warning ring/objective marker로 구현했으나, 공격/위험 범위의 다이아몬드형 외곽선은 실제 화면에서 시각 소음이 크다는 피드백이 있어 대체 문법을 검토한다.
- 경계태세 후속 UX/버그: `오버워치` 사용자 노출 명칭을 `경계태세`로 변경하고, 발동 가이드 텍스트와 경계태세 사망 렌더링 지연을 검증한다 (`TBD-014`, `BUG-001`).
