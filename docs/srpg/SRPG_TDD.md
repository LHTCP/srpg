...
1. 타일 overlay 문법을 실제 플레이 판단 기준으로 재정리한다. 이동/공격/ZOC/오버워치/패링/상호작용이 색상만 다른 채움으로 보이면 북쪽 사격 루트와 남쪽 신호 장치 루트가 묻힌다.
2. 행동 순서 패널을 별도 initiative/turn order tracker로 분리할지 UI 구조를 확정한다.
3. 총기 기본 공격/오버워치/발포 연출의 방향 문법을 분리 검토하고, 8방향 고정으로 보이는 조준 문제를 P2에서 재정의한다.
4. 메이커/맵 에디터 UX에서 엄폐 segment와 상호작용 포인트 편집 방식을 검토한다.
5. 초기 4인 고유 패시브/대표 스킬 최종 수치와 전직 연계를 확정한다.
6. 공용 전투 태그/패링/총기 파급 브릿지 수치 밸런스를 검증한다.
7. 메이커 효과유형 드롭다운 성능과 필드 의미 툴팁은 P3 UX로 재현/설계한다.
