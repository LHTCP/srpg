...
- Delivery workflow가 Actions cache 예산 가드에서 실패했다면 기본 10GiB 한도 중 2GiB 미만만 남았다는 의미다.
...
- Delivery 차단을 풀기 위해 cache 한도를 올리는 것은 유료 전환 리스크가 있으므로, 먼저 오래된 cache 삭제를 우선한다.
