...
현재 저장소는 커스텀 LFS 서버를 사용하지만, public repo에 사설 서버 URL을 직접 노출하지 않기 위해 `.lfsconfig`를 커밋하지 않는다. GitHub Actions standard runner 실행 시간 자체와 별개로, LFS 다운로드는 커스텀 서버의 네트워크 접근성, 인증, 트래픽 한도를 함께 확인한다.
